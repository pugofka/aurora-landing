...
function Modal({ project, initialIndex, onClose }: { project: typeof projects[0], initialIndex: number, onClose: () => void }) {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 backdrop-blur-xl p-4 md:p-10" onClick={onClose}>
            <button className="absolute top-8 right-8 text-white/50 hover:text-white transition-colors z-[110]" onClick={onClose}>
                <X size={40} strokeWidth={1} />
            </button>
            
            <div className="relative w-full h-full flex items-center justify-center" onClick={e => e.stopPropagation()}>
                <div className="relative w-full h-full max-h-[85vh] aspect-[16/9]">
                    <Image
                ...
                {project.images.length > 1 && (
                    <>
                        <button onClick={prev} className="absolute left-0 top-1/2 -translate-y-1/2 w-16 h-16 flex items-center justify-center text-white/30 hover:text-white transition-colors bg-white/5 hover:bg-white/10 rounded-full">
                            <ChevronLeft size={48} strokeWidth={1} />
                        </button>
                        <button onClick={next} className="absolute right-0 top-1/2 -translate-y-1/2 w-16 h-16 flex items-center justify-center text-white/30 hover:text-white transition-colors bg-white/5 hover:bg-white/10 rounded-full">
                ...
                <div className="absolute bottom-0 left-0 right-0 text-center pb-8">
                    <p className="font-display text-2xl uppercase tracking-widest text-white mb-2">{project.title}</p>
                    <p className="font-mono text-sm text-white/40 uppercase">{index + 1} / {project.images.length}</p>
                </div>
            </div>
        </div>
    );
